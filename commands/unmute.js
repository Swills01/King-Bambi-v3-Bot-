...
module.exports = {
    name: 'unmute',
    description: 'Unlocks the group so everyone can send messages (Admins/Creators/Bot only)',
    async execute(sock, m, from) {
        ...
        if (!isOwner && !isAdmin) {
            return sock.sendMessage(from, { text: '❌ *Access Denied:* Only group admins and creators can unmute the group.' }, { quoted: m });
        }

        try {
            await sock.groupSettingUpdate(from, 'not_announcement');
            await sock.sendMessage(from, { text: '🔓 Group has been *unmuted*. Everyone can now send messages.' }, { quoted: m });
        } catch (error) {
            console.error('Error unmuting group:', error);
            await sock.sendMessage(from, { text: '❌ Failed to unmute group. Ensure the bot is an admin.' }, { quoted: m });
        }
    }
};
